...
public class LoveYouForeverRest
{
    ...
    private readonly Dictionary<string, UnityWebRequestAsyncOperation> CachingCount = new Dictionary<string, UnityWebRequestAsyncOperation>();
    // 字典里是url和对应的文本缓存
    ...
    private void SendWebRequestText(string url, Action<string> onCompleted, Action<string, long, string> onError, string body = null, long timeout = DefaultTimeout)
    {
        ...
        if (CachingCount.ContainsKey(url))
        {
            ...
            CachingCount[url].completed += operation =>
            {
